...
#[allow(lint(self_transfer))]
public fun start_run<T>(pool: &StakingPool<T>, clock: &Clock, ctx: &mut TxContext) {
    let session = Session {
        id: object::new(ctx),
        user: ctx.sender(),
...
public fun submit_run_with_proof<T>(
    pool: &mut StakingPool<T>,
    user_data: &mut UserData,
    session: Session,
    steps: u64,
...
#[allow(lint(self_transfer))]
public fun distribute_rewards<T>(pool: &mut StakingPool<T>, clock: &Clock, ctx: &mut TxContext) {
    let current_time = clock.timestamp_ms();
